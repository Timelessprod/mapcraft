...
class District(Area):
	...
	def getRegionsPolygons(polygon):
		...
		points = np.array([[x, y] for x in lineX
					for y in lineY if polygon.contains(Point(x, y))])
...
#		points += np.random.random((len(points), 2)) * (radius / 2)
		...
		regions = [r for r in vor.regions]
...
#		roads = regionsFull.difference(MultiPolygon(regions))
	...
	def __init__(self, polygon, category):
		regionsPoly, _ = District.getRegionsPolygons(polygon)
