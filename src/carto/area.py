...
class Area():
    ...
    def split(self, percentage, direction, inplace=True, new_category=Category.GARDEN):
        ...
        start = np.array(self._polygon.centroid)
            ...
            np.array([diameter * np.cos(direction),
                     diameter * np.sin(direction)])
        ...
        while abs(res[0].area - house_area) > 1:  # 1 meter² error accepted
            # print(".", end="")
            cut = LineString([pt1 + width * orth - diameter *
                             dir, pt2 + width * orth + diameter * dir])
...
if __name__ == "__main__":
    ...
    # house in south, it takes 40 % of the area
